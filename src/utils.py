...
def custom_load_train_batchfnames(text_path, augment_path, batch_size, usenum_text = 3, usenum_augment = 5, trainnum = 100000):
...
def prepare_text_batch(batchfnames, wd=128, ht=128, anglejitter=False):
    img_list = []
    for fname in batchfnames:
        img = Image.open(fname).convert('RGB')
        ori_wd, ori_ht = img.size
        if ori_ht > ht and ori_wd > wd:
